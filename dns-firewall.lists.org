...
#
# Regex needs to include a group/anchor/placeholder named either 'domain', 'entry', 'ip', 'line' or 'regex' (multiple allowed)
# - e.g.: (?P<entry> ...)'
# Regex can start with an add-sign '@' to indicate using a named regex from listregexes file
#
# Exregex is an excluding regex, works same as regex above but filter outs instead of including. Cannot be a shortcut, does not support group/anchor/placeholders.
#
# Example regexes:
...
EasyList-Dutch-White	white	/etc/unbound/easylist-dutch.list.download	/etc/unbound/easylist-dutch-white.list	86400	@ADBLOCK-WHITE
...
EasyList-French-White	white	/etc/unbound/easylist-french.list.download	/etc/unbound/easylist-french-white.list	86400	@ADBLOCK-WHITE
